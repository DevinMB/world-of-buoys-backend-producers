...
class SRADAggregator:
    ...
    def parse_buoy_data_line(self, line):
        ...
        try:
            ...
            lwrad = float(lwrad) if lwrad is not None else None
        except ValueError:
            print(f"Error parsing radiation values in line: {line}")
            return None
    ...
    def run(self):
        ...
        for station_id in buoy_stations:
            ...
            if not self.redis_conn.sismember(available_info_key, 'srad'):
                # print(f"Skipping {station_id} - .srad file not available.")
